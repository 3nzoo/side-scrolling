...
MyGame = ig.Game.extend({
	...
	draw: function() {
		...
		if (this.onMessage) {
			...
			var score = typeof ig.system.currentScore != 'undefined' ? ig.system.currentScore : 0;
			var appendText = ig.ua.mobile ? "Press button to \ncontinue" : " Press 'c' to continue";
			switch(this.messageLevel){
				case 'LevelMessage1':
					var messageText = this.messages[0] + "\n\n\n" + appendText;
					this.font.draw( messageText, cx + 30, top + 130, ig.Font.ALIGN.LEFT);
					this.font.draw( "LEVEL: 1-3", cx - 400, top + 320, ig.Font.ALIGN.LEFT);
					this.font.draw( "SCORE: " + score, cx - 200, top + 320, ig.Font.ALIGN.LEFT);
				break;
				case 'LevelMessage2':
					var messageText = this.messages[1] + "\n\n\n" + appendText;
					this.font.draw( messageText, cx + 30, top + 130, ig.Font.ALIGN.LEFT);
					this.font.draw( "LEVEL: 2-3", cx - 400, top + 320, ig.Font.ALIGN.LEFT);
					this.font.draw( "SCORE: " + score, cx - 200, top + 320, ig.Font.ALIGN.LEFT);
				break;
				case 'LevelMessage3':
					var messageText = this.messages[2] + "\n\n\n" + appendText;
					this.font.draw( messageText, cx + 30, top + 80, ig.Font.ALIGN.LEFT);
					this.font.draw( "LEVEL: 3-3", cx - 400, top + 320, ig.Font.ALIGN.LEFT);
					this.font.draw( "SCORE: " + score, cx - 200, top + 320, ig.Font.ALIGN.LEFT);
				break;
			}
		}
...
MyTitle = ig.Game.extend({
	...
	update: function() {
		...
		if( ig.input.pressed('jump') || ig.input.pressed('shoot') || ig.input.pressed('left') || ig.input.pressed('right') ) {
